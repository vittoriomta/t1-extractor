...
def extract(file):
    ...
    for file in os.listdir(dir):
        ...
        CL_Weight = Weight
        Count += len(Weight)
        CL_Seal = listToString(Seal).replace('1 SIG. OR.', '').replace('1 SIG.', '').replace('---', '').replace('OR.', '')
        CL_Mrn = Mrn.replace('MRN', '')
        CL_Cargo = listToString(Cargo).replace('Imballaggio', '')
